...
def make_conjs(target, regular_exception = False):
    U = []
    L = []

    L1 = list(filter(None, make_conjectures_one(target, regular_exception)))
    L2 = list(filter(None, make_conjectures_two(target, regular_exception)))
    L3 = list(filter(None, make_conjectures_three(target, regular_exception)))
    L4 = list(filter(None, make_conjectures_four(target, regular_exception)))
    for x in L1:
        if x.inequality == " <= ":
            U.append(x)
        else:
